...
async function viewReports(root) {
  ...
  async function render() {
    ...
    container.append(el('div', { class: 'alert alert--info' },
      'ملاحظة الاحتساب: إذا درّب المدرب شخصين في نفس الساعة تُحسب ساعة تدريب واحدة، بينما يُحسب عدد الأشخاص حسب العدد الفعلي — وتُخصم حصة من كل متدرب. والحصة التعويضية تُخصم من رصيد الاشتراك كالعادية — وتبقى موسومة «تعويض» في السجل.'));
  }
