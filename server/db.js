/* ============================================================
   سبورت باور — طبقة البيانات
   مخزن JSON بسيط على القرص (data/db.json) مع بيانات تجريبية.
   ============================================================ */
const fs = require('fs');
...
function hash(password) {
  return crypto.createHash('sha256').update('sp-salt::' + password).digest('hex');
}
...
/* حفظ مؤجل حتى لا نكتب على القرص مع كل طلب */
function save() {
...
function nextId(collection) {
  const d = get();
  const max = d[collection].reduce((m, r) => Math.max(m, r.id), 0);
  return max + 1;
}
...
/* ============================================================
   البيانات التجريبية — أسماء وأرقام واقعية بالعربية
   ============================================================ */
function seed() {
  ...
  const subscriptions = [
    ...
    // اشتراك منتهٍ (للتقارير وقائمة المنتهية)
  ...
  /* حصص منفذة — تشمل مثال «شخصان بنفس الساعة» للمدرب عمر */
  const sessions = [];
  let sid = 1;
  const addSession = (traineeId, trainerId, branchId, date, time, duration, style, notes, weight) => {
    sessions.push({ id: sid++, traineeId, trainerId, branchId, date, time, duration, style, notes: notes || '', weight: weight || null, createdAt: date + 'T' + time + ':00' });
  };
  // الشهر الماضي — عينة
  ...
  // مثال البريف: شخصان بنفس الساعة = ساعة تدريب واحدة وشخصان
  ...
  /* مواعيد Calendar — أيام قادمة من هذا الأسبوع */
  const appointments = [];
  let aid = 1;
  ...
  /* مكتبة التغذية */
  const meals = [
    { id: 1, name: 'شوفان بالحليب والموز', type: 'breakfast', goal: 'muscle', calories: 420, protein: 22, carbs: 62, fat: 9, ingredients: 'شوفان 80غ، حليب قليل الدسم 250مل، موزة، عسل ملعقة صغيرة، قرفة', preparation: 'يُطبخ الشوفان بالحليب 5 دقائق، يُضاف الموز شرائح والعسل والقرفة.', image: null, createdBy: 6 },
    { id: 2, name: 'بيض مسلوق مع توست أسمر', type: 'breakfast', goal: 'loss', calories: 290, protein: 20, carbs: 24, fat: 12, ingredients: 'بيضتان، شريحتا توست أسمر، خيار وطماطم', preparation: 'يُسلق البيض 8 دقائق ويُقدّم مع التوست والخضار.', image: null, createdBy: 6 },
    { id: 3, name: 'صدر دجاج مشوي مع أرز', type: 'lunch', goal: 'muscle', calories: 560, protein: 45, carbs: 58, fat: 12, ingredients: 'صدر دجاج 200غ، أرز بسمتي 150غ مطبوخ، بروكلي، زيت زيتون', preparation: 'يُتبّل الدجاج ويُشوى، يُقدّم مع الأرز والبروكلي المسلوق.', image: null, createdBy: 6 },
    { id: 4, name: 'سلطة تونة', type: 'lunch', goal: 'loss', calories: 330, protein: 32, carbs: 14, fat: 16, ingredients: 'علبة تونة بالماء، خس، ذرة، خيار، ليمون، زيت زيتون ملعقة', preparation: 'تُخلط المكونات وتُتبل بالليمون وزيت الزيتون.', image: null, createdBy: 6 },
    { id: 5, name: 'سمك سلمون مع خضار سوتيه', type: 'dinner', goal: 'maintain', calories: 450, protein: 38, carbs: 18, fat: 24, ingredients: 'فيليه سلمون 180غ، كوسا، فلفل ألوان، بصل، زيت زيتون', preparation: 'يُشوى السلمون 12 دقيقة وتُقلّب الخضار على نار متوسطة.', image: null, createdBy: 6 },
    { id: 6, name: 'زبادي يوناني بالتوت', type: 'snack', goal: 'loss', calories: 160, protein: 15, carbs: 17, fat: 4, ingredients: 'زبادي يوناني 170غ، توت مشكل 80غ', preparation: 'يُضاف التوت فوق الزبادي مباشرة.', image: null, createdBy: 6 },
    { id: 7, name: 'شيك بروتين بزبدة الفستق', type: 'snack', goal: 'muscle', calories: 380, protein: 32, carbs: 28, fat: 15, ingredients: 'مكيال بروتين، حليب 300مل، زبدة فستق ملعقة، موز نصف', preparation: 'تُخلط المكونات في الخلاط 30 ثانية.', image: null, createdBy: 6 },
    { id: 8, name: 'كينوا بالدجاج والأفوكادو', type: 'dinner', goal: 'loss', calories: 410, protein: 34, carbs: 36, fat: 15, ingredients: 'كينوا 120غ مطبوخة، دجاج مشوي 120غ، أفوكادو ربع، جرجير', preparation: 'تُقدّم الكينوا مع شرائح الدجاج والأفوكادو على فرشة جرجير.', image: null, createdBy: 6 },
    { id: 9, name: 'عجة الخضار بالجبن', type: 'breakfast', goal: 'maintain', calories: 340, protein: 21, carbs: 12, fat: 23, ingredients: '3 بيضات، سبانخ، فلفل، جبن أبيض 40غ', preparation: 'تُخفق البيضات مع الخضار وتُطهى على مقلاة غير لاصقة.', image: null, createdBy: 6 },
    { id: 10, name: 'ستيك لحم مع بطاطا مشوية', type: 'lunch', goal: 'muscle', calories: 640, protein: 48, carbs: 45, fat: 26, ingredients: 'ستيك 220غ، بطاطا 200غ، هليون، زبدة قليلة', preparation: 'يُشوى الستيك حسب الرغبة وتُشوى البطاطا بالفرن 25 دقيقة.', image: null, createdBy: 6 },
  ];

  /* خطة غذائية مربوطة بمتدرب (مثال) */
  const mealPlans = [
  ...
  return {
    branches, users, subscriptions, payments, sessions, appointments,
    inbody, meals, mealPlans, notifications,
    tokens: {},
  };
}

module.exports = { get, save, persist, nextId, hash, reseed };
