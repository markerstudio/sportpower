...
const SCHEMA = {
  branches: {
    columns: {
      ...
      freezeLimit: col('int'),
      /* عملة الفرع: صريحة على كل فرع — عمّان بالدينار وفروع الضفة بالشيكل.
         كانت اختيارية والفارغ يتبع عملة النظام، فقلب تغييرُ الإعداد العام
         أرقامَ عمّان معه. الآن يُملأ الفارغ مرة واحدة عند الإقلاع
         (store.backfillBranchCurrencies) ولا يمسّه إعدادٌ عام بعدها. */
      currency: col('text'),
    },
    indexes: [],
  },
  ...
  targets: {
    columns: {
      ...
      /* الهدف الحرّ (metric = custom): «الإدارة تضيف أهدافًا زي ما بدها» —
         اسمُه من الإدارة، ومحقَّقُه يُحدَّث يدويًا لأن النظام لا يقيسه بنفسه */
      label: col('text'),
      actual: col('num'),
    },
